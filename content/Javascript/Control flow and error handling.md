...
>} 
>```
console.log(x); // 2 This outputs `2` because the `var x` statement within the block is in the same scope as the `var x` statement before the block. (In C or Java, the equivalent code would have output `1`.) 
...
**If no match is found:** It then looks for a special "default" option.

- **If there's a `default`:** It runs the code associated with that `default` option. (Even though it's often at the end, it doesn't _have_ to be.)
- **If there's no `default`:** It just skips over the entire `switch` block and continues with the code right after it.

#### break statements

Imagine the `switch` statement is like a **maze**, and each `case` is a different path:

**`break` acts like an "exit" sign:**

Once a `switch` statement finds a matching `case` and runs its code, the `break` statement immediately tells the program: "Okay, I'm done here! Exit the `switch` block and continue with the rest of the program."

**If you _don't_ use `break` (this is called "fall-through"):**

The program will run the code for the matching `case`, but then it **won't stop**. It will keep going and run the code for the _next_ `case` (and the one after that, and so on) until it either hits a `break` statement or reaches the end of the `switch` block.
